...
export async function generateMessageResponse({
    ...
    while (true) {
        try {
            ...
            // Extract reasoning before JSON parsing
            const reasoningMatch = rawResponse.match(
                /Reasoning:\n([\s\S]*?)\n\n```json/,
            );
            elizaLogger.debug("Reasoning extraction:", {
                hasMatch: !!reasoningMatch,
            ...
            if (reasoningMatch?.[1]) {
                if (!parsedContent.reasoning) {
                    parsedContent.reasoning = reasoningMatch[1].trim();
                }
                elizaLogger.debug("Added reasoning to content:", {
                    reasoningLength:
                        typeof parsedContent.reasoning === "string"
                            ? parsedContent.reasoning.length
                            : undefined,
