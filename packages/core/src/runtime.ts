...
export class AgentRuntime implements IAgentRuntime {
    ...
    // Helper function for consistent randomization
...
Text: ${attachment.text}
  `
        ...
        // Enhanced bio randomization
        let bio = this.character.bio || "";
        if (Array.isArray(bio)) {
            const selectedBios = this.getRandomElements(bio, 3);
            bio = selectedBios.join("\n\n"); // Better separation between bio segments
        }

        // Enhanced lore randomization with weighted selection
        let lore = "";
        if (this.character.lore && this.character.lore.length > 0) {
            const loreCount = Math.min(
                5,
                Math.max(2, Math.floor(Math.random() * 4))
            ); // Random 2-5 lore pieces
            const selectedLore = this.getRandomElements(
                this.character.lore,
                loreCount
            );
        ...
        // Enhanced message examples with dynamic selection
        const messageExampleCount = Math.min(
            5,
            Math.max(2, Math.floor(Math.random() * 4))
        ); // Random 2-5 examples
        const formattedCharacterMessageExamples = this.getRandomElements(
        ...
        // Enhanced post examples with dynamic selection
        const postExampleCount = Math.min(
            3,
            Math.max(1, Math.floor(Math.random() * 3))
        ); // Random 1-3 posts
        const formattedCharacterPostExamples = this.getRandomElements(
        ...
        // Enhanced topics with dynamic selection
        const selectedTopics = this.getRandomElements(
        ...
        const topicsString =
            selectedTopics.length > 0
                ? `${
                      this.character.name
                  } is interested in ${selectedTopics.join(", ")}`
                : "";

        // Enhanced adjectives with dynamic selection
        const selectedAdjectives = this.getRandomElements(
            this.character.adjectives || [],
            2
        );
        const adjectivesString = selectedAdjectives.join(" and ");
